...
class TimeLineEventSerializer(serializers.ModelSerializer):
    ...
    date_time = serializers.PrimaryKeyRelatedField(queryset=DateTimeInfo.objects.all())
    
    # Add nested read-only representations for better API responses
    document_detail = serializers.SerializerMethodField()
    date_time_detail = serializers.SerializerMethodField()
    ...
    def get_date_time_detail(self, obj):
        """Return nested date_time details"""
        return DateTimeInfoSerializer(obj.date_time).data
...
class TimeLineEventImportSerializer(serializers.Serializer):
    ...
    """
    document = serializers.DictField(required=True, help_text="Document data (title, content, categories)")
    date_time = serializers.DictField(required=True, help_text="DateTimeInfo data (active_glow, sol, solar_year)")
