...
class SkillActionService(BaseService):
    ...
    def check_skill_capability(self):
        # check character has skill in equipted skills
        self.initiator.has_skill(self.action.skill_id)
        skill = self.action.skill

        SkillCostService(skill).apply(self.initiator.character)
        ...
        if skill.is_defense() or skill.is_buff() or skill.is_heal():
            for ally in self.get_allys().filter(id__in=[target.character.id for target in self.target]):
                suitable_targets.append(ally)
        if skill.is_attack() or skill.is_debuff():
            if self.initiator.character.fight is None:
                raise GameLogicException("Character not in fight")
            for enemy in self.get_enemies().filter(id__in=[target.main_character.id for target in self.target]):
    ...
    def get_allys(self):
        fight = self.initiator.character.fight
        if self.initiator.character in fight.side_a_participants.filter(id=self.initiator.character.id):
    ...
    def check_target_capability(self):
        ...
        for target in self.target:
            if target.character.fight != self.initiator.character.fight:
                continue
            suitable_targets.append(target)
            # check if target is in same or same -1 dimension
            if not (target.character.dimension_id == self.initiator.character.dimension_id \
                    or target.character.dimension_id == (self.initiator.character.dimension_id - 1)):
                continue
